//-----------------------------------------------------------------------
// <copyright file="SwaggerToPlSqlClientCommand.cs" company="NSwag">
...
namespace NSwag.Commands.CodeGeneration
{
    ...
    [Command(Name = "swagger2plsqlclient", Description = "Generates Oracle client code from a Swagger/OpenAPI specification (obsolete: use openapi2csclient instead).")]
    public class OpenApiToPlSqlClientCommand : OpenApiToPlSqlCommandBase<PlSqlClientGeneratorSettings>
    {
        public OpenApiToPlSqlClientCommand() : base(new PlSqlClientGeneratorSettings())
        {
        }
        ...
        public async Task<Dictionary<string, string>> RunAsync()
        {
            return await Task.Run(async () =>
            {
                ...
                var clientGenerator = new PlSqlClientGenerator(document, Settings);
        ...
        private void GenerateImplementation(Dictionary<string, string> result, PlSqlClientGenerator clientGenerator)
        {
        ...
        private void GenerateContracts(Dictionary<string, string> result, PlSqlClientGenerator clientGenerator)
        {
            var savedNamespace = Settings.PlSqlGeneratorSettings.Namespace;
            Settings.PlSqlGeneratorSettings.Namespace = ContractsNamespace;
            result[ContractsOutputFilePath ?? "Contracts"] = clientGenerator.GenerateFile(ClientGeneratorOutputType.Contracts);
            Settings.PlSqlGeneratorSettings.Namespace = savedNamespace;
        }
    }
}
