//-----------------------------------------------------------------------
// <copyright file="SwaggerToPlSqlGeneratorBase.cs" company="NSwag">
...
{
    /// <summary>The PlSql generator base class.</summary>
    public abstract class PlSqlGeneratorBase : ClientGeneratorBase<PlSqlOperationModel, PlSqlParameterModel, PlSqlResponseModel>
    {
        private readonly PlSqlGeneratorBaseSettings _settings;
        private readonly PlSqlTypeResolver _resolver;
        private readonly OpenApiDocument _document;

        /// <summary>Initializes a new instance of the <see cref="PlSqlGeneratorBase"/> class.</summary>
        /// <param name="document">The document.</param>
        /// <param name="settings">The settings.</param>
        /// <param name="resolver">The resolver.</param>
        protected PlSqlGeneratorBase(OpenApiDocument document, PlSqlGeneratorBaseSettings settings, PlSqlTypeResolver resolver)
        ...
        public override string GetTypeName(JsonSchema schema, bool isNullable, string typeNameHint)
        {
            ...
            return Resolver.Resolve(schema.ActualSchema, isNullable, typeNameHint);
                //.Replace(_settings.PlSqlGeneratorSettings.ArrayType + "<", _settings.ResponseArrayType + "<")
                //.Replace(_settings.PlSqlGeneratorSettings.DictionaryType + "<", _settings.ResponseDictionaryType + "<");
        }
        ...
        /// <param name="settings">The settings.</param>
        /// <param name="document">The document </param>
        public static PlSqlTypeResolver CreateResolverWithExceptionSchema(PlSqlGeneratorSettings settings, OpenApiDocument document)
        {
            var exceptionSchema = document.Definitions.ContainsKey("Exception") ? document.Definitions["Exception"] : null;

            var resolver = new PlSqlTypeResolver(settings, exceptionSchema);
        ...
        {
            var model = new PlSqlFileTemplateModel(clientTypes, dtoTypes, outputType, _document, _settings, this, (PlSqlTypeResolver)Resolver);
            var template = _settings.CodeGeneratorSettings.TemplateFactory.CreateTemplate("PlSql", "File", model);
        ...
        {
            var generator = new PlSqlGenerator(_document, _settings.PlSqlGeneratorSettings, _resolver);
